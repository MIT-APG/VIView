...
Start-Up
=======================
To open the tool double click VIView.mlapp in the cloned directory. If this does not work, have MATLAB open and right click VIView.mlapp in the directory window and click "Run".
...
Default Values
=======================
The Pulse Voltage Data field has already been populated with a sample data file. This data file is an experimental voltage waveform from an APG experiment. Other sample data files are provided in the Data folder.
...
* VIView forces the end of the system to be ground as a BC
...
* Please be patient with spark gap inputs, especially if there are multiple. Please simplify the inputs if the computation takes too long (may be too many points, the max time is too long, etc.)

* All loads are constant RC loads and the spark gap's insulating and conductive resistances are hard-coded to model air -- please feel free to change this in the back-end for other gases
...
The first cable is a typical 5 meter cable from your power supply to the resistor. The load is a 50 ohm resistor with no capacitance. 
...
Leave the Max Time and Timesteps as is and leave the Extract Solution Vecs checkbox as is. Now, please click Compute.

The RHS of the GUI should populate within 10-15 seconds. The Voltage vs. Time plot should be a peak at 94 ns and and the current profile should look very similar to the voltage. Since it is a matched load, the plots should show that there are little-to-no reflections. Clicking Play under the bottom-left graph should play a video that shows this fact. Also please notice that a lot of energy is deposited in the load because it is a matched system.

Now, let's add a probe at x = 0 and x = 5 m. Please input 0, 5 (with the space) into the Probe Location field, do not change anything else, and click Compute again. You will see that top-left, top-right, and bottom-right plots show this change appropriately.
...
Click Clear All and make sure that the Sample Gaussian box is checked so that we use the Sample Gaussian.
...
Let's see what the waveforms look like at say, 2m so please put 2 in the Probe Location field. Spark gaps are more computationally costly to model -- this run might take longer than the past cases (~30 secs) so please be patient!
